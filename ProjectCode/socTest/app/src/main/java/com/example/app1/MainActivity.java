...
public class MainActivity extends AppCompatActivity {
    ...
    static boolean IsTCP=true;
    static boolean IsTCPServer=false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Button bnt1 = findViewById(R.id.bnt1);
        final Button bnt2 = findViewById(R.id.bnt2);
        final Button bnt3 = findViewById(R.id.bnt3);
        final Button bnt4 = findViewById(R.id.bnt4);

        final Switch Switch1 = findViewById(R.id.switch1);
        final Switch Switch2 = findViewById(R.id.switch2);

        final TextView text1 = findViewById(R.id.txt1);
        final TextView text2 = findViewById(R.id.txt2);
        ShowText = text2;
        text2.setMovementMethod(ScrollingMovementMethod.getInstance());
        final TextView input1 = findViewById(R.id.input1);
        final TextView input2 = findViewById(R.id.input2);
        final TextView input3 = findViewById(R.id.input3);

        final UDPServer[] UDPServer = {null};
        final TCPServer[] TCPServer = {null};


        final String[] text = {(String) input1.getText().toString()};
        final String[] IP = {(String) input2.getText().toString()};
        final String[] PortStr = {(String) input3.getText().toString()};
        final Integer[] Port = {Integer.valueOf(PortStr[0])};
        final TCPClient[] mThread = {null};

        bnt1.setOnClickListener(//IP显示//
                new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        // TODO Auto-generated method stub
                        String text = MainActivity.getLocalIpAddress();
                        text1.setText("本机IP:" + text);
                    }
                });
        bnt2.setOnClickListener(//TCP服务器//
                new View.OnClickListener() {
                    @Override
                    public void onClick(View v)
                    {
                        ...
                        {
                            if(TCPServer[0] ==null)
                            //UDPServer[0].SetPort(Port[0]);
                            {
                                ShowErro("开启TCP服务器");
                                TCPServer[0] =new TCPServer();
                                PortStr[0] = (String) input3.getText().toString();
                                Port[0] = Integer.valueOf(PortStr[0]);
                                TCPServer[0].SetPort(Port[0]);
                                TCPServer[0].NewAClient();
                            }
                            else
                            {
                                ShowErro("新建一个用户接入口");
        ...
        bnt3.setOnClickListener(//发送按钮//
                new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if(IsTCP)
                        {
                            //mThread[0] = new TCPClient(mHandler, IP, Port,text);
                            text[0] = (String) input1.getText().toString();
                            if(IsTCPServer)
                            {
                                TCPServer[0].SendMSGAllID(text[0]);
                            }
                            else
                            {
                                if(TCPConect)
                                {
                                    mThread[0].setMsg(text[0]);
                                    mThread[0].sendSocket();
                                }
                                else
                                {
                                    IP[0] = (String) input2.getText().toString();
                                    PortStr[0] = (String) input3.getText().toString();
                                    Port[0] = Integer.valueOf(PortStr[0]);
                                    mThread[0] = new TCPClient(IP[0], Port[0], text[0]);
                                    mThread[0].sendSocket();
                                    bnt4.setText("断开");
                                    TCPConect = true;
                                    //mThread[0].ReadIO();
                        ...
                        else
                        {
                            ...
                            PortStr[0] = (String) input3.getText().toString();
                            Port[0] = Integer.valueOf(PortStr[0]);
                            UDPClient.UDPSend(text[0],IP[0],Port[0]);
                        }
                    }
                });
        bnt4.setOnClickListener(//断开按钮
                new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        // TODO Auto-generated method stub
                        if(IsTCP)
                        {
                            ...
                            {
                                ShowErro("关闭TCP服务器");
                                if(TCPServer[0]!=null && TCPServer[0].IsRun())//关闭TCP服务器
                                {
                                    TCPServer[0].CloseServer();
                                }
                                TCPServer[0] = null;
                                IsTCPServer=false;
                            ...
                            {
                                ShowErro("继续打酱油");
                                if(mThread[0]!=null && mThread[0].IsNormal())
                                {
                                    mThread[0].CloseIO();
                                }
                                TCPConect = false;
        ...
        /*
        bnt5.setOnClickListener(//开启TCP服务器的按钮
                new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        // TODO Auto-generated method stub
         ...
         */
        Switch1.setOnCheckedChangeListener(//汉字编码
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked){
                    ...
                    //Todo
                    charset="utf-8";
                    Switch1.setText("UTF8");
                    ShowErro("切换到UTF-8编码");
                }
            }
        });
        Switch2.setOnCheckedChangeListener(//TCP?UDP
                new CompoundButton.OnCheckedChangeListener() {
                    @Override
                    public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                        if (isChecked)
                        ...
                        {
                            //Todo
                            if(mThread[0]!=null && mThread[0].IsNormal())//关闭TCP客户端
                            {
                                mThread[0].CloseIO();
                            }
                            ...
                            if(UDPServer[0]==null)
                            {
                                UDPServer[0] = new UDPServer();////打开UDP接收
                            }
                            PortStr[0] = (String) input3.getText().toString();
                            Port[0] = Integer.valueOf(PortStr[0]);
                            UDPServer[0].SetPort(Port[0]);
    ...
    public static void ShowData(String data)//显示接收到的数据
    {
        t.setToNow(); // 取得系统时间。
        int year = t.year;
        int month = t.month;
        ...
        //AddNorMalText(data,true);

    }
    public static void ShowData2(String data)//显示错误
    {
        t.setToNow(); // 取得系统时间。
        int year = t.year;
        int month = t.month;
    ...
    public static void ShowErro(String data)//显示错误
    {
        t.setToNow(); // 取得系统时间。
        int year = t.year;
        int month = t.month;
    ...
    private static void AddNorMalText(String text,boolean AddBr) {
        if(text==null)
        {
            text="none";
        }
    ...
    private static void AddColorText(String text,String color,boolean AddBr) {
        if(text==null)
        {
            text="none";
        }
        ...
        if(AddBr)
        {
            ctxt=colorText(text + "<br>",color);

        }
        else
        {
            ctxt=colorText(text ,color);
        }
        ShowText.append(ctxt);
    }
    private static Spanned colorText(String text,String color) {
...
class UDPClient {
    ...
    public static void UDPSend(final String content,final String IP,final int Port) {
        ...
        new Thread() {
            ...
            public void run() {
                try {
                    sendBuf = content.getBytes(MainActivity.charset);
                ...
                try {
                    ...
                    while (life) {
                        try {
                            ...
                            ShowData(new String(msg,MainActivity.charset));
                ...
                if(socket==null) {
                    ...
                    else
                    {
                        ...
                        try {
                            ...
                            SetIS(is);
                            os.write(msg.getBytes(MainActivity.charset));
                            os.flush();
                            ShowData2(msg);
                        } catch (IOException e) {
                            ErrStop("发送出错了~(-_-)~");
